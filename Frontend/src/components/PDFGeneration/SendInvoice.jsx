...
export const sendInvoicePDF = async (invoice, returnBlob = false) => {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();

  // Helper function: Format numbers to INR style
  const format = (value) =>
  ...
  try {
    // 1️⃣ Fetch Company Info from Backend
    ...
    // 2️⃣ Draw the full PDF
    await drawPDF(doc, invoice, companyInfo);

    // 3️⃣ Convert PDF to Base64
    const pdfBase64 = doc.output("datauristring").split(",")[1];

    // 4️⃣ If blob is requested, return it
    if (returnBlob) {
    ...
    // 5️⃣ Otherwise, send PDF via API to backend
    const sendRes = await axios.post(`${API_BASE_URL}/api/send/send-invoice`, {
      pdfBase64,
   ...
   async function drawPDF(doc, invoice, companyInfo) {
    // ✍️ Everything you already wrote: headers, QR, tables, etc...
