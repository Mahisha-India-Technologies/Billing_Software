...
export default function GstInvoice() {
  ...
  const handleBarcodeScan = async (e) => {
    if (e.key === "Enter" && barcode.trim() !== "") {
      try {
        ...
        // ✅ Calculate all fields to match modal
        ...
        // ✅ Sync to selectedProducts (for modal)
        ...
        // ✅ Add to main invoice product list
        ...
        setSnackbarMessage("Error fetching product or not found");
        setSnackbarSeverity("error");
        setSnackbarOpen(true);
        setBarcode("");
      }
  ...
  const handleProductChange = (index, field, value) => {
    ...
    if (field === "particular") {
      ...
      updated[index] = {
        ...
        stock_quantity: selected.stock_quantity, // ✅ store full stock here
        amount: amount.toFixed(2),
        priceIncludingGst: priceWithGst.toFixed(2),
      };
      ...
      setSelectedProducts((prev) => ({
        ...
        [selected.product_id]: {
          ...
          gst: selected.gst,
          discount: selected.discount,
          stock_quantity: selected.stock_quantity, // ✅ don't reduce this
          amount: amount.toFixed(2),
          priceIncludingGst: priceWithGst.toFixed(2),
        },
      }));
      ...
      // ✅ New: get the correct stock from either productList or selectedProducts
      const productId = updated[index].product_id;
      const fallbackStock = updated[index].stock_quantity ?? 0;

      const productFromList = productList.find(
        (p) => p.product_id === productId
      );
      const productFromSelected = selectedProducts[productId];

      const stock =
        productFromList?.stock_quantity ??
        productFromSelected?.stock_quantity ??
        fallbackStock;
      ...
      // ✅ stock_quantity remains unchanged — don’t subtract from it
      updated[index].stock_quantity = stock;
  ...
  const handleSaveDraftToLocal = () => {
    ...
    const draftData = {
      customer, // ✅ includes email & whatsapp_number
      products,
      summaryData: updatedSummary,
      ...
      borderRadius: "4px",
              ...
              <Grid sx={{ display: "flex", flexDirection: {xs: 'column', sm: 'row', md: 'row', lg: 'row'}, p: 1, mt: 3, gap: '10px' }}>
                ...
                >
                  ...
                  <Typography sx={{fontWeight: 'bold'}}>Add Products</Typography>
                </Button>
                ...
                >
                  ...
                  <Typography sx={{fontWeight: 'bold'}}>Select Products</Typography>
                </Button>
              </Grid>
            </Box>
          </AnimatedBox>
...
>
  <MenuItem value="Cash" sx={{ backgroundColor: isDark ? "#2b2b2b" : "#fff", color: isDark ? "#fff" : "#000" }}>
    Cash
  </MenuItem>
  <MenuItem value="Card" sx={{ backgroundColor: isDark ? "#2b2b2b" : "#fff", color: isDark ? "#fff" : "#000" }}>
    Card
  </MenuItem>
  <MenuItem value="UPI" sx={{ backgroundColor: isDark ? "#2b2b2b" : "#fff", color: isDark ? "#fff" : "#000" }}>
    UPI
  </MenuItem>
</Select>
  ...
  MenuProps={{
    PaperProps: {
      sx: {
        ...
        boxShadow: `0px 4px 12px ${isDark ? "#000" : "#aaa"}`,
      },
    },
    MenuListProps: {
      sx: {
  ...
  <Box sx={{ flex: 1 }}>
    <Box component="ul" sx={{ listStyle: "none", pl: 0, m: 0 }}>
      {[
        ...
        {
          label: `Discount (${
            discountType === "%" ? `${discount || 0}%` : `₹${discount || 0}`
          })`,
